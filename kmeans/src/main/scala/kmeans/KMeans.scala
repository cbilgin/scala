...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val res = new concurrent.TrieMap[Point, GenSeq[Point]]()
    for(center <- means)
      res.put(center, GenSeq[Point]())

    res ++ (points groupBy (findClosest(_, means)))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    newMeans.isEmpty || ((oldMeans.zip(newMeans)).map( p => p._1.squareDistance(p._2)) reduce (math.max(_,_)) ) <= eta
  }
