...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    ...
    case Fork(l, r, c, w) => (chars(l) ::: chars(r) ::: c).distinct
  }
  ...
  def insert(elem: Fork, l: List[CodeTree]): List[CodeTree] = {
    if (l.isEmpty)
      List(elem)

    else if (weight(elem) > weight(l.head))
      List(l.head) ::: insert(elem, l.tail)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: y :: cs => insert(makeCodeTree(x, y), cs)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeChar(t: CodeTree, b: List[Bit]): List[Char] = t match {
      case Leaf(c, _) => if (b.isEmpty) List(c) else c :: decodeChar(tree, b)
      case Fork(l, r, _, _) =>  if (b.head == 0) decodeChar(l, b.tail)  else  decodeChar(r, b.tail)
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(t: CodeTree)(x: List[Char]): List[Bit] = t match {
      case Fork(l, r, _, _) => if(chars(l).contains(x.head)) 0::encodeChar(l)(x) else 1::encodeChar(r)(x)
      case Leaf(_,_) => if(x.tail.isEmpty) List() else encodeChar(tree)(x.tail)
    }
