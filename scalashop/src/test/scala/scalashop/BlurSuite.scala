...
class BlurSuite extends FunSuite {
  ...
  test("Parellel HorizontalBoxBlur.blur with radius 1 should correctly blur the entire 3x3 image") {
    val w = 3
    val h = 4
    val src = new Img(w, h)
    val dst = new Img(w, h)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 1
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 9; src(1, 2) = 7; src(2, 2) = 8
    src(0, 3) = 0; src(1, 3) = 0; src(2, 3) = 0

    //dst(2,3) = boxBlurKernel(src, 2, 3, 2, true)
  ...
  test("Parallel VerticalBoxBlur.blur with radius 2 should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
